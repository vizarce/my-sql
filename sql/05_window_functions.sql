...
--  12. Combined example — full analytical report
-- =============================================================================
...
-- Running headcount over hire dates (how many employees on board over time)
SELECT
    hire_date,
    employee_id,
...
SELECT
    ...
    d.department_name,
    e.hire_date,
    LAG(CONCAT(e.first_name,' ',e.last_name))  OVER (PARTITION BY e.department_id ORDER BY e.hire_date) AS prev_hired,
    LEAD(CONCAT(e.first_name,' ',e.last_name)) OVER (PARTITION BY e.department_id ORDER BY e.hire_date) AS next_hired
FROM employees e
JOIN departments d ON d.department_id = e.department_id
...
SELECT
    ...
    p.product_name,
    p.price,
    FIRST_VALUE(p.product_name) OVER w  AS cheapest_in_category,
    FIRST_VALUE(p.price)        OVER w  AS cheapest_price,
...
WINDOW w AS (PARTITION BY p.category_id ORDER BY p.price
             ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)
...
-- ===========================================================================
-- 11. NAMED WINDOW (WINDOW clause) — define once, reuse across SELECT
-- ===========================================================================

-- Salary stats relative to department, defined as a named window
SELECT
    CONCAT(e.first_name, ' ', e.last_name)            AS employee_name,
    d.department_name,
    e.salary,
    ROUND(AVG(e.salary)  OVER dept_window, 2)         AS dept_avg,
    MIN(e.salary)        OVER dept_window              AS dept_min,
    MAX(e.salary)        OVER dept_window              AS dept_max,
    RANK()               OVER dept_salary_rank         AS salary_rank
FROM employees e
JOIN departments d ON d.department_id = e.department_id
WINDOW
    dept_window      AS (PARTITION BY e.department_id),
    dept_salary_rank AS (PARTITION BY e.department_id ORDER BY e.salary DESC)
ORDER BY d.department_name, salary_rank;
